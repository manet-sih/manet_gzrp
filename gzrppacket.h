...
class GzrpPacket: public ns3::Header{
	...
	public:
		...
		void setNeighbourZones(std::set<uint32_t>& zones);
		void addZone(uint32_t zone);
		bool deleteZone(uint32_t zone);
		void setMetric(uint32_t);
		ns3::Ipv4Address getSrcIp();
		uint32_t getSeqNo()const;
		uint32_t getZoneId()const;
		uint8_t getNumKnownZones() const;
		bool getNeighbourZones(std::set<uint32_t>& zones) const;
